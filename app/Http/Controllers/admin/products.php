...
class products extends Controller
{
    public function insert(Request $request)
    {
        ...
        if ($categoryId != null) {
            ...
            $newProduct->AttachFileProduct = url('/') . '/AttachFileProduct/' . $AttachFileProduct->getClientOriginalName();
            $newProduct->path_image = url('/') . '/ProductImage/' . $ProductImage->getClientOriginalName();
            $newProduct->userId = Auth::user()->id;

            $newProduct->save();

            return redirect('admin/addProduct')->with('sucuss', $newProduct->id);

        } else {
            return redirect('admin/addProduct')->with('sucuss', 0);
        }
    ...
    public function delete($id)
    {
        ...
        if ($check == null) {
            return redirect('admin/editProduct')->with('deleteStatus', 'true');
        } else {
            return redirect('admin/editProduct')->with('deleteStatus', 'false');
        }
    }
    ...
    public function edit(Request $request)
    {
        ...
        return redirect('admin/editProduct-sub' . $request->id)->with('updateStatus', 'true');
    ...
    public function Search(Request $request)
    {
        ...
        if ($request->search_with != null) {
            if ($request->search_with == 'title' || $request->search_with == 'price') {
                $SearchProduct = product::where($request->search_with, 'like', '%' . $request->search_product . '%')->get();
            } else {
                return redirect('admin/editProduct')->with('error', 'دوباره تلاش کنید!');
            }
        } else {
            $SearchProduct = product::where('title', 'like', '%' . $request->search_product . '%')->get();
        }

        return view('admin.editProduct', compact('SearchProduct'));
    }
