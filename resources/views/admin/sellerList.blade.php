...
$SellerRequest = \App\User::where('seller', '1')->paginate(15);
...
@section('MainContent')
        ...
        <div class="row" style="direction: rtl;">
            <div class="col-lg-12 col-md-12 col-sm-12 pull-right">
                <h1 class="page-header visible-lg visible-md visible-sm" style="margin-top:0; padding-top:0;">لیست
                    فروشندگان</h1>
                <h2 class="page-header visible-xs">لیست فروشندگان - {{ 'سلام ' . \Illuminate\Support\Facades\Auth::user()->name}}</h2>
            </div>
        </div>
        ...
        <div class="Table">
            ...
            <div class="row" style="background: transparent;padding-right: 0;">
                ...
                @if(\Illuminate\Support\Facades\Session::has('message'))
                    @if(\Illuminate\Support\Facades\Session::get('message') == 'true')
                        <div class="alert alert-success" style="direction: rtl;">
                            {{'درخواست فروشندگی با موفقیت لغو شد!'}}
                        </div>
                    @else
                        <div class="alert alert-danger" style="direction: rtl;">
                            {{'در هنگام لغو درخواست فروشندگی خطایی رخ داد؛ دوباره تلاش کنید!'}}
                        </div>
                    @endif
                @endif
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 pull-right"
                    ...
                    <div>
                        <form action="{{url('admin/searchSellerList')}}" method="post">
                            {{csrf_field()}}
                ...
                @foreach($SellerRequest as $temp)
                    ...
                    <div class="row tableContent mainCategory">
                        ...
                        <div class="col-lg-12 col-md-12 col-sm-12" style="padding: 0;">
                            <a href="{{url('admin/MoreDetailSeller' . $temp->id )}}"
                               onclick="getId({{$temp->id}})" data-toggle="modal"
                               data-target="#exampleModalCenter">{{'لغو مجوز فروشندگی'}}</a>
                        </div>
                    </div>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLongTitle">لغو مجوز فروشندکی</h5>
                    </div>
                    <div class="modal-body">
                        <span>
                            {{'آیا از لغو مجوز فروشندگی این فروشنده اطمینان دارید؟'}}
                        </span>
                    </div>
    ...
    <script>
        ...
        function deleteCategory(confirmId) {
            if (confirmId == '1') {
                window.location.href = 'DenySeller' + SellerId;
            }
        }
